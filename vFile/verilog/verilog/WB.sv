...
always_comb begin : signal_assignment
    ...
    if(mem_wb_bus.valid)begin
        ...
        if(mem_wb_bus.memory_re)begin
            ...
            shifted_ram_data = ram_data >> ({30'b0 , mem_wb_bus.addr[1:0]} << 3);   // 移动0/8/16/24位
             case (mem_wb_bus.func3)
                3'b010  : w_data_loadType  = ram_data;                                  // lw
        ...
        wb_id_bus.w_data = (mem_wb_bus.memory_re) ? w_data_loadType :
                                        mem_wb_bus.w_data;
//如果访问读内存，那w_data是计算的读内存地址，即addr，而ram_data才是真正的要写回rd的内容
    end
end
